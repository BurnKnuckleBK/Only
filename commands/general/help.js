...
exports.run = async (client, message, args) => {
  let prefixs = db.get(`prefix_${message.guild.id}`)
      if(prefixs === null) prefixs = prefix;
  ...
  if (!args[0]) {
    ...
    if (!owners.includes(message.author.id)) module = client.helps.array().filter(x => !x.hide);
