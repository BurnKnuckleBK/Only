...
exports.run = async (client, message, args) => {
        ...
        // No reason
        if (!args[1]) {
            return message.reply("Please provide a reason to kick.").then(msg => {
                msg.delete({ timeout: 5000 })
            })
        }
        ...
        if (!message.member.hasPermission("KICK_MEMBERS")) {
            return message.reply("❌ You do not have permissions to kick members. Please contact a staff member").then(msg => {
                msg.delete({ timeout: 5000 })
            })
        }
        ...
        if (!message.guild.me.hasPermission("KICK_MEMBERS")) {
            return message.reply("❌ I do not have permissions to kick members. Please contact a staff member").then(msg => {
                msg.delete({ timeout: 5000 })
            })
        }
        ...
        await message.channel.send(promptEmbed).then(async msg => {
            ...
            if (emoji === "✅") {
                ...
                message.reply(`Kick canceled.`).then(msg => {
                    msg.delete({ timeout: 5000 })
                })
            }
        });
